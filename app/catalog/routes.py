...
def edit_book(book_id):
    book = Book.query.get(book_id)
    form = EditBookForm(obj=book)   # with obj=book the data is prefilled in fields while updating
    ...
    if form.validate_on_submit():
        ...
        db.session.add(book)
        db.session.commit()
        flash('Book Detail Edited Successfully')
        return redirect(url_for('main.display_books'))
    return render_template('edit_book.html', form=form, name=book.title)

@main.route('/create/book/<pub_id>', methods=['GET', 'POST'])
@login_required
def create_book(pub_id):
    form = CreateBookForm()
    form.pub_id.data = pub_id   # pre filled of publisher id
